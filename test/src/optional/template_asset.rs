...
use daml_type_rep::built_in_types::{DamlParty, DamlText, DamlOptional};
...
pub async fn exercise_get_view(
    ...
    if let Some(CommandResult::ExerciseResult(value)) = result.get(0) {
        ...
        info!("Exercise GetView result extracted from LAPI value: {:#?}", Asset::from_lapi_value(value));
    }
    Ok(())
}
...
pub struct Asset {
    issuer: DamlParty,
    owner: DamlParty,
    name: DamlOptional<DamlText>,
}

impl Asset {
    pub fn new(issuer: String, owner: String, name: Option<String>) -> Self {
        Asset {
            issuer: DamlParty::new(&issuer),
            owner: DamlParty::new(&owner),
            name: DamlOptional::new(name.map(DamlText::new)),
        }
    }
}
